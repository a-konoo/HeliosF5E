...
namespace GadrocsWorkshop.Helios.Controls.F5E
{
    ...
    [HeliosControl("Helios.F5E.IFFMasterSelector", "IFFMasterSelector", "F-5E", typeof(GeneralPurposeKnobBaseRenderer), HeliosControlFlags.NotShownInUI)]
    public class IFFMasterSelector : GeneralPurposeKnobBase
    {
        private static readonly NLog.Logger Logger = NLog.LogManager.GetCurrentClassLogger();

        public IFFMasterSelector(string name, Point posn, Size size,
        ...
        public override int CurrentPosition
        {
            ...
            set
            {
                ...
                if (_fromButtonInput != 0)
                {
                    ...
                    var sendIndex = SendValueMasterSelector(diffValue, oldValue);
                    var sendValue = Positions[sendIndex].SendValue;
                    _sendValueTrigger.FireTrigger(new BindingValue(sendValue));
                    _currentPosition = sendIndex;
                }
        ...
        private int SendValueMasterSelector(int diffValue, int oldValue)
        {
            if (diffValue < 0)
            {
                if (PullReleaseKnob)
                ...
                return oldValue < 5 ? oldValue+1 : 5;
            }
            if (oldValue == 1)
            {
                return PullReleaseKnob ? 0: 1;
            }
            if (PullReleaseKnob)
            {
                PullReleaseKnob = false;
            }
            return oldValue <= 1 ? 1 : oldValue - 1;
        }
