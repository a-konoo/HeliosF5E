...
namespace GadrocsWorkshop.Helios.Controls
{
    ...
    [HeliosControl("Helios.Base.FlipLever", "FlipLever", "F-5E", typeof(FlipLeverRenderer))]
    public class FlipLever : HeliosVisual
    {
        ...
        private bool _isSelectLever;
        private bool _isSelectAndDrag;
        ...
        public FlipLever() : base("FlipLever", new Size(432, 264))
        {
            Positions = new FlipLeverStepCollection();
            AnimationFrameImageNamePattern = "{Helios}/Images/F-5E/DragChute/DragChute_01.png";
            var positionFileName = Path.GetFileName(AnimationFrameImageNamePattern);
            Positions.Add(new FlipLeverPosition(this, 0, "0", 1, 1, 100, 1, 160, 20));
            Positions.Add(new FlipLeverPosition(this, 1, "1", 2, 15, 100, 2, 0, 20));
            Positions.Add(new FlipLeverPosition(this, 2, "2", 15, 16, 100, 3, 230, 20));
            Positions.Add(new FlipLeverPosition(this, 3, "3", 17, 18, 100, 0, 230, 20));
        ...
        public bool IsSelectLever
        {
            get => _isSelectLever;
            set
            {
                if (_isSelectLever == value)
                {
                    return;
                }
                bool oldValue = _isSelectLever;
                _isSelectLever = value;
                OnPropertyChanged("IsSelectLever", oldValue, value, true);
            }
        }

        public bool IsSelectAndDrag
        {
            get => _isSelectAndDrag;
            set
            {
                if (_isSelectAndDrag == value)
                {
                    return;
                }
                bool oldValue = _isSelectAndDrag;
                _isSelectAndDrag = value;
                OnPropertyChanged("IsSelectAndDrag", oldValue, value, true);
            }
        }
        ...
        {
            if (!isDragging && IsSelectAndDrag)
            {
                IsSelectLever = !IsSelectLever;
                _selectedTrigger.FireTrigger(new BindingValue(IsSelectLever));
                var angle = Positions[CurrentPositionIndex].DragAngle;
                _angleTrigger.FireTrigger(new BindingValue(angle));
            }
        }
        ...
        public override void MouseDrag(Point location)
        {
            ...
            var position = Positions[CurrentPosition] as FlipLeverPosition;
            if (Math.Sqrt(diff.X * diff.X + diff.Y * diff.Y) > position.DragDistance)
            {
                isDragging = true;
                var dragAngle = position.DragAngle;
                var product = diff.X * Math.Cos(dragAngle / 180 * Math.PI) + diff.Y * Math.Sin(dragAngle / 180 *Math.PI);
                if (product > 0)
                {
                    var sendValue = position.SendValue;
                    _statusTrigger.FireTrigger(new BindingValue(sendValue));
                }
            }
        }
        ...
        public override void WriteXml(XmlWriter writer)
        {
            ...
            writer.WriteElementString("IsSelectAndDrag", Convert.ToString(IsSelectAndDrag));
            ...
            foreach (FlipLeverPosition position in Positions)
            {
                ...
                writer.WriteAttributeString("DragDistance", position.DragDistance.ToString(CultureInfo.InvariantCulture));
        ...
        public override void ReadXml(XmlReader reader)
        {
            ...
            Boolean.TryParse(reader.ReadElementString("IsSelectAndDrag"), out bool _isSelectAndDrag);
            IsSelectAndDrag = _isSelectAndDrag;
            ...
            if (!reader.IsEmptyElement)
            {
                ...
                while (reader.NodeType != XmlNodeType.EndElement)
                {
                    ...
                    Int32.TryParse(reader.GetAttribute("DragDistance"), out int dragDistance);
